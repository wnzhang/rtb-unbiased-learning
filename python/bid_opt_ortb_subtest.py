...
def bidding_ortb(pctr, base_ctr, c, l):
    return int(math.sqrt(pctr * c * l / base_ctr + c * c) - c)
...
def simulate_one_bidding_strategy_with_parameter(wyzps, tcost, proportion, algo, para):
    ...
    for wyzp in wyzps:
        ...
        if algo == "const":
            ...
            bid = bidding_ortb(pctr, original_ctr, optimal_c, para)
        else:
            print 'wrong bidding strategy name'
            sys.exit(-1)

        bids += 1 * weight
        if bid > winprice: # (budget - cost) >= bid * weight > winprice * weight:
            imps += 1 * weight
            clks += clk * weight
    ...
    if clks > prop_algo_optimal_perf[proportion][algo]:# and cost < budget:
    ...
    elif clks == prop_algo_optimal_perf[proportion][algo]:# and cost < budget:
...
result_folder = '../results/bid-opt-ortb-subtest/'
campaign = sys.argv[1]
...
wyzps_train.reverse()
random.shuffle(wyzps_train)

original_ecpc = total_cost_train * 1. / clk_num_train
#print 'Base ecpc:', original_ecpc
...
budget_proportions = [16] # , 1, 0.5, 0.25] # , 32, 8]
...
ortb_paras = [t * 0.1 for t in range(50, 120, 1)]  # only for lambda
